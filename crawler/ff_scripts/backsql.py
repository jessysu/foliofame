...
#DB = {"host" : "db", "user" : "root", "passwd" : "test", "db" : "ff_dev", "port": 3306} 


logging.basicConfig(level=logging.WARN)

BACKUP_DIR = "%s/backups" % "/backup_mysql"#os.path.dirname(os.path.abspath(__file__))#os.path.dirname(__file__)
...
def _run_backup(file_name):
   cmd = "%(mysqldump)s --databases %(database)s --tables %(tables)s --host=%(host)s --port=%(port)d --user=%(user)s -p%(password)s > %(log_dir)s/%(file)s" % {
       'mysqldump' : MYSQL_CMD,
       'host': DB["host"],
       'port': DB["port"],
       'user' : DB["user"],
